#!/usr/bin/env python3
"""
Validation utility for thorax configuration files.

Mirrors the upper-limb validator by ensuring all cross-file references resolve,
including attachment points, muscle heads, muscles, nerves, arteries, and actions.
"""
...
CONFIG_DIR = Path(__file__).resolve().parents[4] / "config" / "anatomy" / "thorax"
FILES = {
...
def validate(configs: Dict[str, dict]) -> List[ValidationError]:
    ...
    for ap in attachment_points:
        bone = ap.get("bone")
        if bone and bone not in bone_ids:
            errors.append(
                ValidationError(
                    section="attachment_points",
                    context=f"attachment '{ap['id']}' references unknown bone",
                    missing=[bone],
                )
            )
    ...
    for bone in bones:
        missing = [
            aid for aid in bone.get("attachments", []) if aid not in attachment_ids
        ]
        if missing:
            errors.append(
                ValidationError(
                    section="bones",
                    context=f"bone '{bone['id']}' references missing attachments",
                    missing=missing,
                )
            )
    ...
    for muscle in muscles:
        missing_insertions = [
            ins for ins in muscle.get("insertion", []) if ins not in attachment_ids
        ]
        ...
        missing_heads = [
            hid for hid in muscle.get("heads", []) if hid not in muscle_head_ids
        ]
        if missing_heads:
            errors.append(
                ValidationError(
        ...
        missing_antagonists = [
            aid for aid in muscle.get("antagonists", []) if aid not in contractile_ids
        ]
...
def main(argv: Sequence[str]) -> int:
    ...
    print("Thorax anatomy configuration is consistent.")
    return 0
