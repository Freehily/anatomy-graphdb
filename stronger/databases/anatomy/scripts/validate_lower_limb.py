#!/usr/bin/env python3
"""
Validation utility for lower limb configuration files.
"""
...
CONFIG_DIR = Path(__file__).resolve().parents[1] / "configs" / "lower_limb"
FILES = {
...
def validate(configs: Dict[str, dict]) -> List[ValidationError]:
    ...
    for muscle in muscles:
        ...
        missing_heads = [
            head_id for head_id in muscle.get("heads", []) if head_id not in muscle_head_ids
        ]
        if missing_heads:
            errors.append(
                ValidationError(
        ...
        missing_antagonists = [
            ...
            if antagonist and antagonist not in contractile_ids
        ]
...
def main(argv: Sequence[str]) -> int:
    ...
    print("Lower limb anatomy configuration is consistent.")
    return 0
